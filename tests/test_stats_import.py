...
def day(name: str, tokens: int = 10) -> dict:
    return {
        "day": name,
        "day_stats": {
...
class TestValidateImport(unittest.TestCase):
    ...
    def test_rejects_duplicate_and_unordered_days(self):
        with self.assertRaises(StatsImportInvalid) as ctx:
    ...
    def test_rejects_a_malformed_day_key(self):
    ...
    def test_requires_day_stats_to_agree_with_the_day(self):
    ...
    def test_rejects_counts_that_are_not_whole_non_negative_numbers(self):
    ...
    def test_allows_the_nullable_measurements(self):
    ...
    def test_strips_unknown_keys_instead_of_storing_them(self):
        doc = document("2026-01-01")
    ...
    def test_caps_the_number_of_days(self):
...
class TestCleanSource(unittest.TestCase):
    ...
    def test_strips_paths_control_characters_and_bounds_length(self):
        for raw, absent in [
            ("../../etc/passwd", "/"),
            ("a\\b", "\\"),
            ("name\ninjected", "\n"),
...
class TestStatsImportService(unittest.TestCase):
    ...
    def test_get_is_none_before_anything_is_imported(self):
        self.assertIsNone(self.service.get())
        self.assertEqual(self.service.count(), 0)

    def test_replace_then_read_back_in_chronological_order(self):
    ...
    def test_out_of_order_input_is_refused_rather_than_silently_sorted(self):
        """A good error must not be defeated by a reader that re-sorts quietly."""
        with self.assertRaises(StatsImportInvalid) as ctx:
            self.service.replace(document("2026-01-02", "2026-01-01"))
        self.assertEqual(ctx.exception.code, "out_of_order")

    def test_replace_replaces_the_previous_document(self):
    ...
    def test_a_rollback_leaves_the_previous_import_intact(self):
        self.service.replace(document("2026-01-01"))
    ...
    def test_clear_reports_how_many_days_it_removed(self):
        self.service.replace(document("2026-01-01", "2026-01-02"))
    ...
    def test_exported_at_is_kept_alongside_each_stored_day(self):
